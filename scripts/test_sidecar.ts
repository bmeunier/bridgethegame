...
async function testSidecar() {
  ...
  try {
    ...
    const responseWithDiarization = await deepgramClient.transcribeFromUrl(
      audioUrl,
      {
        model: "general",
        punctuate: true,
    ...
    const envelopeWithDiarization = deepgramClient.parseResponse(
      episodeId + "-with-diarization",
      responseWithDiarization,
    );
    ...
    console.log(
      `   Deepgram speakers: ${envelopeWithDiarization.deepgram_speakers?.length || 0}`,
    );

    // Verify canonical fields remain null
    const canonicalFieldsNull =
      envelopeWithDiarization.words.every((w) => w.speaker === null) &&
      envelopeWithDiarization.utterances.every((u) => u.speaker === null);

    console.log(
      `   Canonical speaker fields null: ${canonicalFieldsNull ? "✅" : "❌"}`,
    );

    if (
      envelopeWithDiarization.deepgram_speakers &&
      envelopeWithDiarization.deepgram_speakers.length > 0
    ) {
      ...
      envelopeWithDiarization.deepgram_speakers
        ...
        .forEach((segment, i) => {
          console.log(
            `     ${i + 1}. ${segment.speaker} [${segment.start.toFixed(1)}-${segment.end.toFixed(1)}s]`,
          );
        });
    }
    ...
    const responseWithoutDiarization = await deepgramClient.transcribeFromUrl(
      audioUrl,
      {
        model: "general",
        punctuate: true,
    ...
    const envelopeWithoutDiarization = deepgramClient.parseResponse(
      episodeId + "-without-diarization",
      responseWithoutDiarization,
    );

    console.log("✅ Transcription without diarization complete");
    console.log(`   Words: ${envelopeWithoutDiarization.words.length}`);
    console.log(
      `   Utterances: ${envelopeWithoutDiarization.utterances.length}`,
    );
    console.log(
      `   Paragraphs: ${envelopeWithoutDiarization.paragraphs.length}`,
    );
    console.log(
      `   Deepgram speakers: ${envelopeWithoutDiarization.deepgram_speakers?.length || 0}`,
    );

    // Verify sidecar field is absent when diarization disabled
    const sidecarAbsent =
      envelopeWithoutDiarization.deepgram_speakers === undefined;
    ...
    console.log(
      `  - Sidecar field present: ${envelopeWithDiarization.deepgram_speakers ? "✅" : "❌"}`,
    );
    console.log(
      `  - Speaker segments found: ${envelopeWithDiarization.deepgram_speakers?.length || 0}`,
    );
    ...
    if (envelopeWithDiarization.deepgram_speakers) {
      const uniqueSpeakers = new Set(
        envelopeWithDiarization.deepgram_speakers.map((s) => s.speaker),
      );
      console.log(
