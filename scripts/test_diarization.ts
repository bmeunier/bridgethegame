...
async function testDiarization() {
  ...
  try {
    ...
    const firstChannelWords = response.results.channels[0].alternatives[0].words;
    const wordsWithSpeakers = firstChannelWords?.filter(w => w.speaker !== undefined) || [];
    ...
    if (wordsWithSpeakers.length > 0) {
      ...
      wordsWithSpeakers.slice(0, 10).forEach((word, i) => {
        console.log(`  ${i + 1}. "${word.word}" [${word.start}-${word.end}s] → Speaker ${word.speaker}`);
      });
    }

    // Check for utterances with speakers
    const utterances = response.results.channels[0].alternatives[0].utterances;
    const utterancesWithSpeakers = utterances?.filter(u => u.speaker !== undefined) || [];
    ...
    if (utterancesWithSpeakers.length > 0) {
      ...
      utterancesWithSpeakers.slice(0, 5).forEach((utterance, i) => {
        ...
        console.log(`  ${i + 1}. Speaker ${utterance.speaker} [${utterance.start}-${utterance.end}s]: "${text}..."`);
      });
    }
    ...
    if (topLevelUtterances && topLevelUtterances.length > 0) {
      ...
      const withSpeakers = topLevelUtterances.filter(u => u.speaker !== undefined);
    ...
    console.log('Channel[0] alternative[0] fields:', Object.keys(response.results.channels[0].alternatives[0]));
