...
async function testDiarizationWorkflow() {
  ...
  for (const [speakerKey, segments] of Object.entries(clusters)) {
    console.log(
      `   ✓ ${speakerKey}: ${segments.length} segments, ${segments.reduce((total, s) => total + (s.end - s.start), 0).toFixed(1)}s total`,
    );

    // Test representative segment selection
    const representative = selectRepresentativeSegment(segments);
    console.log(
      `     → Representative: ${representative.start}s - ${representative.end}s (${(representative.end - representative.start).toFixed(1)}s)`,
    );
  }
  ...
  const enrichedTranscript = enrichTranscript(
    mockTranscript,
    mockDiarization,
    mockSpeakerMap,
  );
  ...
  enrichedTranscript.forEach((segment, index) => {
    console.log(
      `   ${index + 1}. [${segment.start}s-${segment.end}s] ${segment.speaker} (${segment.speaker_confidence || "N/A"}): "${segment.text}"`,
    );
  });
  ...
  console.log(
    `     - Identified speakers: ${audit.clusters.filter((c) => c.mappedTo).length}/${audit.clusters.length}`,
  );
  ...
  console.log(
    `   ✓ Enriched path: ${diarizationCompleteEvent.data.s3_enriched_path}`,
  );
  console.log(
    `   ✓ Audit path: ${diarizationCompleteEvent.data.s3_audit_path}`,
  );

  console.log("\n✅ Plan 4 Integration Test Complete!");
  console.log("\n📊 Summary:");
  console.log(
    `   • ${enrichedTranscript.length} transcript segments processed`,
  );
  console.log(`   • ${Object.keys(clusters).length} speakers detected`);
  console.log(
    `   • ${audit.clusters.filter((c) => c.mappedTo).length} speakers identified`,
  );
  console.log(
    `   • ${audit.clusters.filter((c) => !c.mappedTo).length} unknown speakers`,
  );
