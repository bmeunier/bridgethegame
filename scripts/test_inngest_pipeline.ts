...
async function testInngestPipeline() {
  ...
  try {
    ...
    try {
      const healthResponse = await axios.get(`${APP_URL}/api/inngest`, {
        timeout: 2000,
      });
    ...
    try {
      const inngestResponse = await axios.get(`${INNGEST_URL}`, {
        timeout: 2000,
      });
    ...
    if (transcriptExists) {
      ...
      if (envelope.episode_id !== episodeId) {
        console.log(
          `   ❌ Episode ID mismatch: ${envelope.episode_id} !== ${episodeId}`,
        );
        allTestsPassed = false;
      } else {
      ...
      if (envelope.deepgram_speakers && envelope.deepgram_speakers.length > 0) {
        console.log(
          `   ✅ Deepgram speakers sidecar present: ${envelope.deepgram_speakers.length} segments`,
        );

        // Verify sidecar format
        const validFormat = envelope.deepgram_speakers.every(
          (seg: any) =>
            seg.speaker &&
            seg.speaker.startsWith("dg-") &&
        ...
        console.log(
          "   ⚠️  No deepgram_speakers sidecar field (diarization may be disabled)",
        );
      }
    }
    ...
    if (allTestsPassed) {
      ...
      console.log("\nPlease check:");
      console.log(
        "1. Both servers are running (npm run inngest-dev & npm run dev)",
      );
