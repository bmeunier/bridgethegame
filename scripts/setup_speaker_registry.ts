...
import { saveSpeakerRegistry } from '../src/lib/speaker-utils';
...
async function createPyannoteVoiceProfile(
  ...
  try {
    ...
    console.log(`   ✓ Voiceprint stored at: s3://${process.env.S3_BUCKET_NAME}/${voiceprintKey}`);

    return referenceId;

  } catch (error) {
    console.error(`   ❌ Failed to create voice profile for ${profileName}:`, error);
    throw error;
  }
}
...
async function setupSpeakerRegistry() {
  ...
  try {
    ...
    const registryPath = path.join(__dirname, '../config/speaker-registry.json');
    const registryConfig = JSON.parse(fs.readFileSync(registryPath, 'utf8'));

    console.log(`   ✓ Loaded registry for podcast: askthegame`);
    console.log(`   ✓ Found speaker: ${Object.keys(registryConfig.askthegame)[0]}`);
    ...
    console.log(`   ✓ Registry uploaded to: s3://${process.env.S3_BUCKET_NAME}/${registryKey}`);
    ...
    console.log(`   ✓ Found ${Object.keys(loadedRegistry).length} registered speakers`);

    for (const [key, info] of Object.entries(loadedRegistry)) {
      console.log(`   ✓ ${key}: ${info.displayName} (threshold: ${info.threshold})`);
    }

    console.log('\n✅ Speaker Registry Setup Complete!');
    console.log('\n📋 Summary:');
    console.log(`   • Registry location: s3://${process.env.S3_BUCKET_NAME}/${registryKey}`);
    console.log(`   • Speakers configured: ${Object.keys(updatedRegistry.askthegame).length}`);
