...
async function debugDiarization() {
  ...
  try {
    ...
    console.log(`Words with speaker field: ${hasSpeakerInWords ? 'YES' : 'NO'}`);
    ...
    console.log(`Utterances with speaker field: ${hasSpeakerInUtterances ? 'YES' : 'NO'}`);

    if (hasSpeakerInUtterances && utterances.length > 0) {
      const firstUtteranceWithSpeaker = utterances.find((u: any) => 'speaker' in u);
      console.log('Sample utterance speaker:', firstUtteranceWithSpeaker?.speaker);
    }
    ...
    console.log(`Has deepgram_speakers: ${envelope.deepgram_speakers ? 'YES' : 'NO'}`);
