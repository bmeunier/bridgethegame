...
async function checkPipelineStatus() {
  ...
  try {
    ...
    try {
      ...
      const ourEvent = recentEvents.find(
        (e: any) =>
    ...
    try {
      ...
      const pipelineRuns = runs.filter(
        (r: any) => r.event_data?.data?.episode_id === episodeId,
      );
    ...
    console.log(
      `   Normalized transcript: ${transcriptExists ? "✅ Exists" : "❌ Not found"}`,
    );
    console.log(
      `   Raw transcript: ${rawExists ? "✅ Exists" : "❌ Not found"}`,
    );
    ...
    if (transcriptExists) {
      ...
      console.log(
        `   Deepgram speakers (sidecar): ${envelope.deepgram_speakers ? `✅ ${envelope.deepgram_speakers.length} segments` : "❌ Not present"}`,
      );

      // Verify canonical fields are null
      if (envelope.words && envelope.words.length > 0) {
        const allWordsNull = envelope.words.every(
          (w: any) => w.speaker === null,
        );
        console.log(
          `   Canonical speaker fields null: ${allWordsNull ? "✅" : "❌"}`,
        );
      }

      if (envelope.deepgram_speakers && envelope.deepgram_speakers.length > 0) {
        console.log(`\n   Sample speaker segments:`);
        envelope.deepgram_speakers
          .slice(0, 3)
          .forEach((seg: any, i: number) => {
            console.log(
              `     ${i + 1}. ${seg.speaker} [${seg.start}-${seg.end}s]`,
            );
          });
      }
    }
