...
async function getEpisodeList() {
  ...
  try {
    ...
    response.data.episodes?.forEach((episode: any, index: number) => {
      ...
      console.log(`   Published: ${new Date(episode.publish_time * 1000).toLocaleDateString()}`);
      console.log('');
    });

    if (response.data.episodes?.length > 0) {
      console.log(`\nTo test transcription, run:`);
      console.log(`npx tsx scripts/test_full_pipeline.ts ${response.data.episodes[0].id}`);
    }

  } catch (error: any) {
    console.error('Failed to get episodes:', error.response?.data || error.message);
