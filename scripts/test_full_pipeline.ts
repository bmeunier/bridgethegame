...
async function testFullPipeline() {
  ...
  try {
    ...
    try {
      // Call Deepgram
      const response = await deepgramClient.transcribeFromUrl(
        episode.media_url,
        {
          model: "general",
          punctuate: true,
      ...
      console.log(
        `   Processing speed: ${(response.metadata.duration / transcriptionTime).toFixed(2)}x realtime\n`,
      );
      ...
      console.log(
        `   Words per minute: ${Math.round((envelope.words.length / response.metadata.duration) * 60)}\n`,
      );
      ...
      if (process.env.S3_BUCKET_NAME) {
        ...
        const rawKey = StorageClient.getTranscriptKey(
          episode.id,
          "deepgram_raw",
        );
        ...
        console.log(
          `   Transcript: s3://${process.env.S3_BUCKET_NAME}/${transcriptKey}`,
        );
      ...
      console.log(
        `Audio Duration: ${response.metadata.duration}s (${Math.round(response.metadata.duration / 60)} min)`,
      );
      ...
      console.log(
        `Average Confidence: ${(envelope.words.reduce((sum, w) => sum + w.confidence, 0) / envelope.words.length).toFixed(3)}`,
      );
      console.log(
        `Words per Minute: ${Math.round((envelope.words.length / response.metadata.duration) * 60)}`,
      );
      ...
      const lowConfidenceWords = envelope.words.filter(
        (w) => w.confidence < 0.5,
      ).length;
      if (lowConfidenceWords > 0) {
        console.warn(
          `⚠️  Warning: ${lowConfidenceWords} words with low confidence (<0.5)`,
        );
      }
