...
/**
 * Rate limiting delay for backfill mode
...
export const ingestEpisode = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    if (event.data.mode === "backfill") {
      await step.sleep("rate-limit-delay", "2s"); // 2 second delay between backfill episodes
    }

    // Fetch episode metadata from Podbean
    const episode = await step.run("fetch-episode-metadata", async () => {
      try {
        const episodeData = await podbeanClient.getEpisode(event.data.episode_id);
