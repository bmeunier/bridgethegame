  ...
  async ({ event, step }) => {
    ...
    const deepgramMetadata = await step.run("deepgram-transcribe", async () => {
      try {
        ...
        const detectedLanguage = response.results.channels[0].alternatives[0]
          .transcript
        ...
        const utteranceCount =
          response.results.channels[0].alternatives[0].utterances?.length || 0;
    ...
    const transcriptSummary = await step.run(
      ...
      async () => {
        try {
          ...
          const deepgramResponse = await storage.loadJson<DeepgramApiResponse>(
          ...
          const normalizedKey = StorageClient.getTranscriptKey(
            episode_id,
            "deepgram",
          );
