...
describe('S3-First Pattern Torture Test', () => {
  ...
  describe('6000+ Segments Torture Test', () => {
    it('should handle 6000 diarization segments without step output errors', async () => {
      ...
      const result = await safeStepOutput(diarizationKey, massiveDiarization, {
        segments: 6000,
        source: 'pyannote_precision2',
      });

      // Verify S3 save was called
      expect(mockSaveJson).toHaveBeenCalledWith(diarizationKey, massiveDiarization);
    ...
    it('should handle 8000 enriched transcript segments', async () => {
      ...
      const massiveEnriched = Array(8000).fill(null).map((_, i) => ({
        start: i * 5.4,
        end: (i * 5.4) + 5.2,
      ...
      const result = await safeStepOutput(enrichedKey, massiveEnriched, {
        segments: 8000,
        identified: 7200,
      });
    ...
    it('should handle complex speaker map with many near-misses', async () => {
      ...
      const nearMisses = Array(200).fill(null).map((_, i) => ({
      ...
      const mapResult = await safeStepOutput(speakerMapKey, speakerMap);
      const missesResult = await safeStepOutput(nearMissesKey, nearMisses);
      ...
      expect(Buffer.byteLength(JSON.stringify(missesResult))).toBeLessThan(1024);
    });
  });
  ...
  describe('Minimal Step Result Pattern', () => {
    it('should create minimal results for all steps', () => {
      ...
      expect(diarizationResult.keys.diarization).toBe(keys.diarizationRaw(episodeId));
      expect(Buffer.byteLength(JSON.stringify(diarizationResult))).toBeLessThan(512);
      ...
      expect(Buffer.byteLength(JSON.stringify(speakerResult))).toBeLessThan(512);
      ...
      expect(Buffer.byteLength(JSON.stringify(enrichmentResult))).toBeLessThan(512);
  ...
  describe('Extreme Edge Cases', () => {
    it('should handle 12-hour episode with 12000 segments', async () => {
      const extremeDiarization = {
        segments: Array(12000).fill(null).map((_, i) => ({
          start: i * 3.6,
          end: (i * 3.6) + 3.5,
      ...
      const result = await safeStepOutput(diarizationKey, extremeDiarization);
    ...
    it('should handle deeply nested audit artifacts', async () => {
      const complexAudit = {
        clusters: Array(100).fill(null).map((_, i) => ({
          ...
          metadata: {
            ...
            energy_profile: Array(10).fill(null).map(() => Math.random()),
          },
        })),
        totalSegments: 12000,
        source: 'pyannote_precision2' as const,
        nearMisses: Array(300).fill(null).map((_, i) => ({
          clusterKey: `SPEAKER_${i % 100}`,
      ...
      const result = await safeStepOutput(auditKey, complexAudit);
    ...
    it('should handle Unicode and special characters', async () => {
      ...
      const mockSaveJson = jest.spyOn(storageSafe, 'saveJson').mockResolvedValue();
      const result = await safeStepOutput('unicode-test', unicodeData);
