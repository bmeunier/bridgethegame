...
describe("Safe Step Output", () => {
  ...
  describe("safeStepOutput", () => {
    it("should save data to storage and return minimal metadata", async () => {
      ...
      const result = await persistStepOutput("test-key", testData, {
        extra: "metadata",
      });
    ...
    it("should force storage for large payloads", async () => {
      ...
      const largeArray = Array(200000).fill("x");
      const largeData = { data: largeArray.join("") };

      const result = await persistStepOutput("large-key", largeData);
    ...
    it("should handle save errors gracefully", async () => {
      ...
      await expect(persistStepOutput("error-key", testData)).rejects.toThrow(
        "S3 save failed",
      );
    });
  });
  ...
  describe("minimalStepResult", () => {
    ...
    it("should work with empty stats", () => {
      const result = minimalStepResult("EP456", {
