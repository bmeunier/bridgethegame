...
jest.mock("../src/lib/speaker-utils", () => ({
  ...
  PyannoteStorageKeys: {
    ...
    getAuditArtifactsKey: (id: string) =>
      `transcripts/${id}/pyannote_audit.json`,
  },
}));
...
describe("🔥 LIVE FIX PROOF: Real Diarization Function", () => {
  ...
  test.skip("🎯 PROOF: Actual function runs without step output size errors", async () => {
    ...
    (groupSegmentsBySpeaker as jest.Mock).mockReturnValue({
      SPEAKER_00: Array(100).fill({
        start: 1.0,
        end: 3.0,
        speaker: "SPEAKER_00",
      }),
      SPEAKER_01: Array(100).fill({
        start: 4.0,
        end: 6.0,
        speaker: "SPEAKER_01",
      }),
    ...
    (getAudioClipUrl as jest.Mock).mockResolvedValue(
    ...
    (identifySpeaker as jest.Mock)
      ...
      .mockResolvedValueOnce({ confidence: 0.9 }); // Fourth speaker match
    ...
    const mockStep = {
      run: jest.fn(async (name: string, fn: Function) => {
        ...
        console.log(
          `✅ Step "${name}" completed with result size: ${JSON.stringify(result).length} bytes`,
        );
        return result;
      }),
    ...
    try {
      ...
      functionResult = await diarizeEpisode.handler({
        event: testEvent,
        step: mockStep,
      });
    ...
    expect(functionResult).toHaveProperty(
      "episode_id",
      "LIVE-PROOF-TEST-EPISODE",
    );
    ...
    console.log(
      "✅ LIVE FIX PROOF PASSED: Real function executed without step output size errors!",
    );
    console.log(
      `📊 Final function result: ${JSON.stringify(functionResult, null, 2)}`,
    );
  });
  ...
  test("🔄 PROOF: Feature flag enables future migration path", async () => {
    ...
    try {
      ...
      jest.resetModules();
      const { enforceStepOutputLimit: safeStepOutput } = await import(
        "../src/lib/safe_step_output"
      );
      ...
      console.log(
        "✅ Future migration path works: Native limits bypass our validation",
      );
    } finally {
  ...
  test("⚡ PROOF: S3-first pattern prevents step output bloat", async () => {
    ...
    console.log(
      `📏 Massive diarization size: ${massiveSize} bytes (${(massiveSize / 1024).toFixed(1)}KB)`,
    );
    ...
    console.log(
      "✅ S3-first pattern proven: Large data → S3, small metadata → step output",
    );
  });
});
