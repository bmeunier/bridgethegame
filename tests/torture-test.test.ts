...
describe("🔥 TORTURE TEST: Step Output Size Limits", () => {
  test("🎯 TORTURE TEST: 12-hour marathon episode with 6,000 segments", () => {
    ...
    expect(() =>
      enforceStepOutputLimit(registryMetadata, "load-speaker-registry"),
    ).not.toThrow();

    const registrySize = JSON.stringify(registryMetadata).length;
    console.log(
      `✅ Registry metadata size: ${registrySize} bytes (${(registrySize / 1024).toFixed(1)}KB)`,
    );
    ...
    console.log(
      `✅ Diarization metadata size: ${diarizationSize} bytes (${(diarizationSize / 1024).toFixed(1)}KB)`,
    );
    ...
    expect(() =>
      enforceStepOutputLimit(speakerMetadata, "cluster-speaker-identification"),
    ).not.toThrow();

    const speakerSize = JSON.stringify(speakerMetadata).length;
    console.log(
      `✅ Speaker metadata size: ${speakerSize} bytes (${(speakerSize / 1024).toFixed(1)}KB)`,
    );
    ...
    console.log(
      `✅ Enrichment metadata size: ${enrichmentSize} bytes (${(enrichmentSize / 1024).toFixed(1)}KB)`,
    );
    ...
    const artifactsMetadata = {
      ...
      audit_path:
        "transcripts/TORTURE-TEST-12HOUR-MARATHON/pyannote_audit.json",
    ...
    expect(() =>
      enforceStepOutputLimit(artifactsMetadata, "save-artifacts"),
    ).not.toThrow();
    ...
    const finalResult = {
      ...
      s3_enriched_path:
        "transcripts/TORTURE-TEST-12HOUR-MARATHON/enriched.json",
      s3_audit_path:
        "transcripts/TORTURE-TEST-12HOUR-MARATHON/pyannote_audit.json",
    ...
    console.log(
      `✅ Final result size: ${finalSize} bytes (${(finalSize / 1024).toFixed(1)}KB)`,
    );
    expect(finalSize).toBeLessThan(1024); // Should be under 1KB

    console.log(
      "🎉 TORTURE TEST PASSED! All step outputs are under 1KB limit.",
    );
  });
  ...
  test("💀 TORTURE TEST: What would BREAK without our fix (massive data)", () => {
    ...
    expect(() =>
      enforceStepOutputLimit(massiveDiarizationResult, "massive-torture-test"),
    ...
    console.log(
      `💀 Massive result size: ${massiveSize} bytes (${(massiveSize / 1024 / 1024).toFixed(1)}MB)`,
    );
  ...
  test("🌪️ EXTREME TORTURE: 24-hour marathon with 12,000 segments", () => {
    ...
    const extremeDiarizationMetadata = {
      ...
      storage_key:
        "transcripts/EXTREME-TORTURE-24HOUR-ULTRA-MARATHON/diarization.json",
      source: "pyannote",
      segments_count: 12000, // Double the torture
      total_duration: 86400, // 24 hours
      speakers_detected: Array(20)
        .fill(null)
        .map((_, i) => `SPEAKER_${i.toString().padStart(2, "0")}`),
      cluster_count: 156,
    ...
    expect(() =>
      enforceStepOutputLimit(
        extremeDiarizationMetadata,
        "extreme-pyannote-diarization",
      ),
    ).not.toThrow();

    const extremeSize = JSON.stringify(extremeDiarizationMetadata).length;
    console.log(
      `✅ EXTREME metadata size: ${extremeSize} bytes (${(extremeSize / 1024).toFixed(1)}KB)`,
    );
    expect(extremeSize).toBeLessThan(2048); // Even extreme cases should be under 2KB

    console.log(
      "🎉 EXTREME TORTURE TEST PASSED! Even 24-hour episodes are safe!",
    );
  });
  ...
  test("🎪 CIRCUS TEST: Validate createSafeStepResult with torture data", () => {
    ...
    expect(tortureSafeResult).toHaveProperty(
      "episode_id",
      "TORTURE-TEST-12HOUR-MARATHON",
    );
    ...
    console.log(
      `✅ Safe result size: ${safeResultSize} bytes (${(safeResultSize / 1024).toFixed(1)}KB)`,
    );
    expect(safeResultSize).toBeLessThan(1024);

    console.log(
      "🎪 CIRCUS TEST PASSED! createSafeStepResult works with complex metadata!",
    );
  });
  ...
  test("🏆 FINAL BOSS: Test all torture scenarios combined", () => {
    ...
    scenarios.forEach((scenario, index) => {
      ...
      console.log(
        `🎯 Testing scenario ${index + 1}: ${scenario.name} (${scenario.segments} segments)`,
      );
      expect(() =>
        enforceStepOutputLimit(metadata, `final-boss-scenario-${index + 1}`),
      ).not.toThrow();
    ...
    console.log(
      "🎉🎉🎉 TORTURE TEST SUITE COMPLETE - FIX IS BULLETPROOF! 🎉🎉🎉",
    );
  });
});
