...
describe('Diarization Retry Simulation', () => {
  ...
  describe('Step 1 Retry: Diarization saved but step fails', () => {
    it('should recover by reloading diarization from S3', async () => {
      ...
      let callCount = 0;
      jest.spyOn(storageSafe, 'saveJson').mockImplementation(async (key, data) => {
      ...
      jest.spyOn(storageSafe, 'mustLoadJson').mockResolvedValue(mockDiarization);

      // Verify that the retry can load the saved diarization
      const loaded = await storageSafe.mustLoadJson(diarizationKey, 'diarization');
  ...
  describe('Step 2 Retry: Speaker identification partially complete', () => {
    it('should reload diarization and complete identification', async () => {
      ...
      jest.spyOn(storageSafe, 'mustLoadJson')
      ...
      jest.spyOn(speakerUtils, 'getSpeakerRegistry').mockResolvedValue(mockRegistry);

      // Mock clustering
      jest.spyOn(pyannoteLib, 'groupSegmentsBySpeaker').mockReturnValue({
        SPEAKER_0: mockDiarization.segments.filter(s => s.speaker === 'SPEAKER_0'),
        SPEAKER_1: mockDiarization.segments.filter(s => s.speaker === 'SPEAKER_1'),
      });

      // Mock representative segment selection
      jest.spyOn(pyannoteLib, 'selectRepresentativeSegment')
        .mockImplementation((segments) => segments[0]);

      // Mock audio clip URL generation
      jest.spyOn(speakerUtils, 'getAudioClipUrl')
        .mockResolvedValue('https://audio.example.com/clip');

      // First attempt: Identify first speaker, then fail
      let identifyCallCount = 0;
      jest.spyOn(pyannoteLib, 'identifySpeaker')
        .mockImplementation(async () => {
          identifyCallCount++;
          if (identifyCallCount === 1) {
            return { matches: true, confidence: 0.92, referenceId: 'ref_hormozi' };
          }
      ...
      const reloadedDiarization = await storageSafe.mustLoadJson(diarizationKey, 'diarization');
  ...
  describe('Step 3 Retry: Enrichment interrupted', () => {
    it('should reload all data and complete enrichment', async () => {
      ...
      jest.spyOn(storageSafe, 'tryLoadJson')
        .mockImplementation(async (key: string) => {
      ...
      const mockEnrichedSegments = mockTranscript.utterances.map((utt, i) => ({
        ...
        speaker_confidence: mockSpeakerMap[`SPEAKER_${i % 2}`]?.confidence || null,
        source: 'pyannote_precision2' as const,
      }));

      jest.spyOn(pyannoteLib, 'enrichTranscript').mockReturnValue(mockEnrichedSegments);

      // First attempt: Enrichment starts but fails midway
      let enrichCallCount = 0;
      const originalEnrich = pyannoteLib.enrichTranscript;
      jest.spyOn(pyannoteLib, 'enrichTranscript')
      ...
      try {
        // First attempt fails
        pyannoteLib.enrichTranscript(mockTranscript.utterances, mockDiarization, mockSpeakerMap);
      } catch (error) {
        // Expected failure
      }

      // Retry succeeds by reloading data
      const transcript = await storageSafe.mustLoadJson(keys.transcript(episodeId), 'transcript');
      const diar = await storageSafe.tryLoadJson(keys.diarizationRaw(episodeId));
      const spkMap = await storageSafe.tryLoadJson(keys.speakerMap(episodeId)) ?? {};
  ...
  describe('Fallback Scenario: Pyannote fails, Deepgram fallback', () => {
    it('should handle Pyannote failure and use Deepgram fallback', async () => {
      ...
      jest.spyOn(pyannoteLib, 'diarize').mockRejectedValue(new Error('Pyannote API down'));
      ...
      jest.spyOn(speakerUtils, 'getDeepgramDiarizationFallback')
  ...
  describe('Data Consistency: S3-first pattern', () => {
    it('should always save to S3 before returning step results', async () => {
      const saveJsonSpy = jest.spyOn(storageSafe, 'saveJson').mockResolvedValue();
      ...
      // Save all data to S3
      await Promise.all([
        storageSafe.saveJson(keys.diarizationRaw(episodeId), testData.diarization),
      ...
      expect(saveJsonSpy).toHaveBeenCalledWith(keys.diarizationRaw(episodeId), testData.diarization);
      expect(saveJsonSpy).toHaveBeenCalledWith(keys.speakerMap(episodeId), testData.speakerMap);
      expect(saveJsonSpy).toHaveBeenCalledWith(keys.nearMisses(episodeId), testData.nearMisses);
      expect(saveJsonSpy).toHaveBeenCalledWith(keys.enriched(episodeId), testData.enriched);
    });
